...
def allocate(call_list: CallForElevator, b: Building, output):
    ...
    for i in call_list:
        ...
        for j in b.list_elevators:
            ...
            if j.is_empty():
                ...
                for k in range(len(j.floor_mng)):
                    ...
                    #         delay = tmp_delay
                    #         break
                    if k < len(j.floor_mng) - 1:
                        if j.floor_mng[k] < i.src < j.floor_mng[k + 1] and can_the_elevator_stop(i, j, k):
                            tmp_on_board = j.times[k] + math.ceil(j.close_time + j.start_time + (abs(j.floor_mng[k] - i.src))/j.speed + j.stop_time + j.open_time)
                            call_t, tmp_end_time, tmp_delay, tmp_idx_dst = calc_time_2(i, j, k)
                            if call_t < min_time:
                                idx_src = k + 1
                                idx_dst = tmp_idx_dst
                                best_elv = j.id
                        ...
                        if j.floor_mng[k] > i.src > j.floor_mng[k + 1] and can_the_elevator_stop(i, j, k):
                            tmp_on_board = j.times[k] + math.ceil(j.close_time + j.start_time + (abs(j.floor_mng[k] - i.src)) / j.speed + j.stop_time + j.open_time)
                            call_t, tmp_end_time, tmp_delay, tmp_idx_dst = calc_time_3(i, j, k)
                            if call_t < min_time:
                                idx_src = k + 1
                                idx_dst = tmp_idx_dst
                                best_elv = j.id
                                ...
                                delay = tmp_delay
                                break
                        if i.src > j.floor_mng[k] > j.floor_mng[k - 1] and j.floor_mng[k +1] < j.floor_mng[k]:
                            tmp_on_board = j.times[k] + math.ceil(j.close_time + j.start_time + (
                                abs(j.floor_mng[k] - i.src)) / j.speed + j.stop_time + j.open_time)
                            call_t, tmp_end_time, tmp_delay, tmp_idx_dst = calc_time_4(i, j, k, tmp_on_board)
                            if call_t < min_time:
                                idx_src = k + 1
                                idx_dst = tmp_idx_dst
                                best_elv = j.id
                        ...
                        if i.src < j.floor_mng[k] < j.floor_mng[k - 1] and j.floor_mng[k +1] > j.floor_mng[k]:
                            tmp_on_board = j.times[k] + math.ceil(j.close_time + j.start_time + (
                                abs(j.floor_mng[k] - i.src)) / j.speed + j.stop_time + j.open_time)
                            call_t, tmp_end_time, tmp_delay, tmp_idx_dst = calc_time_5(i, j, k, tmp_on_board)
                            if call_t < min_time:
                                idx_src = k + 1
                                idx_dst =
                    ...
                    call_t, tmp_on_board, tmp_end_time = check_call_time_0(i, j)
                    if call_t < min_time:
        ...
        changes_before_add(i, j, idx_src, idx_dst, delay, end_time)
        b.list_elevators[best_elv].floor_mng.insert(idx_src, i.src)
        b.list_elevators[best_elv].times.insert(idx_src, on_board)

        b.list_elevators[best_elv].floor_mng.insert(idx_dst + 1, i.dst)
        b.list_elevators[best_elv].times.insert(idx_dst + 1, end_time)
        i.data[5] = best_elv
    ...
    for j in b.list_elevators:
        writer.writerow(j.floor_mng)
        writer.writerow(j.times)
    out_file.close()
    ...
    if call.dir == 1 and elev.floor_mng[k - 1] < elev.floor_mng[k]:
        while k + 1 < len(elev.floor_mng):
            if elev.floor_mng[k + 1] == call.dst:
                end_t = elev.times[k+1] + delay
                return end_t - call.call_time, end_t, delay, k
            ...
            #If the destination is in the middle of a existing work-line
            ...
            if call.dst > elev.floor_mng[k + 1]:
                k = k + 1
        end_t = elev.times[k] + delay +math.ceil(close_t + start_t + (abs(elev.floor_mng[k] - call.dst)) + stop_t +open_t)
        call_t = end_t - call.call_time
        return call_t, end_t, delay, k
...
def calc_time_2(call: CallForElevator, elev: Elevator, k):
    speed = elev.speed
    open_t = elev.open_time
    close_t = elev.close_time
    start_t = elev.start_time
    stop_t = elev.stop_time
    delay = math.ceil(stop_t + open_t) + math.ceil(start_t + close_t)
    ...
    if call.dir == 1:
        while k + 1 < len(elev.floor_mng):
            ...
            if elev.floor_mng[k + 1] == call.dst:
                end_t = elev.times[k+1] + delay
                return end_t - call.call_time, end_t, delay, k + 1
            # If the current destination is closer then the next existing destination of the elevator
            if call.dst < elev.floor_mng[k + 1] or elev.floor_mng[k] > elev.floor_mng[k+1]:
                end_t = elev.times[k] + delay + math.ceil(close_t + start_t + (abs(elev.floor_mng[k] - call.dst))/speed + open_t +stop_t)
                call_t = end_t - call.call_time
                return call_t, end_t, delay, k
            # if the existing destination is lower than the current call destination go to the next call destination
            if call.dst > elev.floor_mng[k + 1]:
                k = k + 1
                continue
            end_t = elev.times[k] + delay + math.ceil(close_t + start_t + (abs(elev.floor_mng[k] - call.dst)) / speed + open_t + stop_t)
    ...
    if call.dir == -1:
        while k + 1 < len(elev.floor_mng) and elev.floor_mng[k + 1] > elev.floor_mng[k]:
            k = k + 1
        while k + 1 < len(elev.floor_mng):
            if call.dst == elev.floor_mng[k + 1]:
                end_t = delay + elev.times[k + 1]
...
def calc_time_3(call: CallForElevator, elev: Elevator, k):
    speed = elev.speed
    open_t = elev.open_time
    close_t = elev.close_time
    start_t = elev.start_time
    ...
    if call.dir == -1:
        while k + 1 < len(elev.floor_mng):
            ...
            # If the current destination is closer then the next existing destination of the elevator
            if call.dst > elev.floor_mng[k + 1] or elev.floor_mng[k] < elev.floor_mng[k + 1]:
                end_t = elev.times[k] + delay + math.ceil(
                    close_t + start_t + (abs(elev.floor_mng[k] - call.dst)) / speed + open_t + stop_t)
                call_t = end_t - call.call_time
                return call_t, end_t, delay, k
            # if the existing destination is lower than the current call destination go to the next call destination
            if call.dst < elev.floor_mng[k + 1]:
...
def calc_time_4(call: CallForElevator, elev: Elevator, k, tmp_on_board):
    speed = elev.speed
    open_t = elev.open_time
    close_t = elev.close_time
    start_t = elev.start_time
    stop_t = elev.stop_time
    if call.dir == 1:
        end_t = tmp_on_board + math.ceil(close_t + start_t + (abs(call.src - call.dst))/speed + stop_t + open_t)
        delay = end_t + math.ceil(close_t + start_t + (abs(elev.floor_mng[k+1] - call.dst))/speed + stop_t + open_t) - elev.times[k+1]
        call_t = end_t - call.call_time
        return call_t, end_t, delay, k + 1
    delay = elev.times[k] + math.ceil(
        close_t + start_t + (abs(elev.floor_mng[k] - call.src))/speed + stop_t + open_t) + math.ceil(
        close_t + start_t + (abs(elev.floor_mng[k + 1] - call.src))/speed + stop_t + open_t) - elev.times[k + 1]
    if call.dst > elev.floor_mng[k + 1]:
        end_t = tmp_on_board + math.ceil(close_t + start_t + (abs(call.dst - call.src))/speed + stop_t + open_t)
        call_t = end_t - call.call_time
        return call_t, end_t, 0, k + 1
    while k+1 < len(elev.floor_mng):
        if call.dst == elev.floor_mng[k+1]:
            end_t = delay + elev.times[k+1]
            call_t = end_t - call.call_time
            return call_t, end_t, delay, k+1
        if call.dst > elev.floor_mng[k+1] or elev.floor_mng[k]<elev.floor_mng[k+1]:
            end_t = elev.times[k] + delay + math.ceil(close_t + start_t + (abs(call.dst - elev.floor_mng[k])/speed) + stop_t + open_t)
            call_t = end_t - call.call_time
            return call_t, end_t, delay, k+1
        if call.dst < elev.floor_mng[k+1]:
            k = k+1
...
def calc_time_5(call: CallForElevator, elev: Elevator, k, tmp_on_board):
    speed = elev.speed
    open_t = elev.open_time
    close_t = elev.close_time
    start_t = elev.start_time
...
def changes_before_add(curr: CallForElevator , elev: Elevator, idx_src, idx_dst, delay, end_time):
    speed = elev.speed
    open_t = elev.open_time
    close_t = elev.close_time
    start_t = elev.start_time
    stop_t = elev.stop_time
    if idx_dst < len(elev.floor_mng):
        for i in range(idx_src, idx_dst):
            if elev.times[i] + delay > elev.times[i]:
                elev.times[i] = elev.times[i] + delay
        delay = math.ceil(end_time + close_t + start_t + (abs(curr.dst - elev.floor_mng[idx_dst]))/speed + stop_t + open_t) - elev.times[idx_dst]
        for i in range(idx_dst, len(elev.floor_mng)):
            if elev.times[i] + delay > elev.times[i]:
                elev.times[i] = elev.times[i] + delay
...
def check_call_time_0(call: CallForElevator, elev: Elevator):
    ...
    if elev.is_empty():
        if call.src == 0:
            ...
            src_t = math.ceil(call.call_time) + math.ceil(close_t + start_t + (abs(call.src)/speed) + stop_t + open_t)
            dst_t = src_t + math.ceil(close_t + start_t + (abs(call.src - call.dst)/speed) + stop_t + open_t)
            call_t = dst_t - src_t
            return src_t, dst_t, call_t
    else:
        if elev.times[-1] <= call.call_time:
            src_t = math.ceil(call.call_time) + math.ceil(close_t + start_t + (abs(call.src - elev.floor_mng[-1]) / speed) + stop_t + open_t)
            dst_t = src_t + math.ceil(close_t + start_t + (abs(call.src - call.dst) / speed) + stop_t + open_t)
            call_t = dst_t - src_t
            return src_t, dst_t, call_t
        else:
            src_t = elev.times[-1] + math.ceil(close_t + start_t + (abs(call.src - elev.floor_mng[-1]) / speed) + stop_t + open_t)
            dst_t = src_t + math.ceil(close_t + start_t + (abs(call.src - call.dst) / speed) + stop_t + open_t)
            call_t = dst_t - src_t
            return src_t, dst_t, call_t
...
if __name__ == '__main__':
    ex1('data\\Ex1_input\\Ex1_Buildings\\B5.json', 'data\\Ex1_Calls_case_2_a.csv', 'out.csv')
